...
# Check and calculate constraint counts
# Counts will be used by multiplying with a lambda and gamma parameter
# and then added to the fitness function which is getAEP()
def Calculate_Constraints(turb_coords, turb_diam):
    """
    -**-THIS FUNCTION SHOULD NOT BE MODIFIED-**- :/ Sorry Had to

    Checks if the turbine configuration satisfies the two
    constraints:(i) perimeter constraint,(ii) proximity constraint
    Note that this function does quantifies the amount by which
    the constraints are violated if any.
    ...
    # Count for perimeter constraint
    count_per = 0
    # Count for proximity constraint
    count_prox = 0
    # create a shapely polygon object of the wind farm
    farm_peri = [(0, 0), (0, 4000), (4000, 4000), (4000, 0)]
    farm_poly = Polygon(farm_peri)
    ...
    # checks if for every turbine perimeter constraint is satisfied.
    # calculates number of perimeter constraints for the positions of turbines
    for turb in turb_coords:
        turb = Point(turb)
        inside_farm = farm_poly.contains(turb)
        correct_clrnc = farm_poly.boundary.distance(turb) >= bound_clrnc
    ...
    # checks if for every turbines proximity constraint is satisfied.
    # calculates number of proximity constraints that for the positions of turbines
    for i, turb1 in enumerate(turb_coords):
        for turb2 in np.delete(turb_coords, i, axis=0):
            if np.linalg.norm(turb1 - turb2) < 4 * turb_diam:
                count_prox = count_prox + 1
                continue
    return (count_per + count_prox)
